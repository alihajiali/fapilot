...
# Named connections override DATABASE_URL for "default" when provided.
# DATABASES = {"default": "sqlite://db.sqlite3", "analytics": "sqlite://analytics.sqlite3"}
...
Run these commands from this directory in an environment with Fapilot installed:

```bash
fapilot startapp users
```

Add `"apps.users.apps.UsersConfig"` to `INSTALLED_APPS` in `config/settings.py`,
then define your models in `apps/users/models.py` and routes in `apps/users/api.py`.

```bash
fapilot runserver
```

The default router prefix is `/api/users`. Interactive API docs are at `/docs`.
`DEBUG=True` creates missing tables for local development; use Aerich migrations
for managed schema changes. For a fresh database with `DEBUG=False`:
...
After changing models, run `fapilot makemigrations --app users`, review the generated
migration, and apply it with `fapilot migrate --app users`.
Python settings take precedence over `.env`; remove conflicting literals to use
environment values. Configure `DATABASES` and `DATABASE_APPS` for multiple databases.

Read the [Fapilot documentation](https://github.com/alihajiali/fapilot/blob/main/docs/index.md)
for the complete walkthrough, settings, authentication, testing, and deployment guides.
"""
